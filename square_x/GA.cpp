...
#define num_of_iteration 10000     // 迭代次数
...
Status cross_over(vector<Point> &random_chosen)
{
    ...
    for (int i = 0; i < population_size / 2; i++)
    {
        ...
        if (cross_probability >= cross_g)
        {
            ...
            // 存储两个剪切下来的片段，以建立映射
            vector<int> cut_part1;
            vector<int> cut_part2;
            ...
            {
                // 依次序保存交换部分，准备 PMX
                cut_part1.push_back(random_chosen[chrome1].Get_Bit(j));
                cut_part2.push_back(random_chosen[chrome2].Get_Bit(j));

                // 依次序交换
                random_chosen[chrome1].Set_Bit(j, random_chosen[chrome2].Get_Bit(j));
            ...
            {
                vector<int>::iterator cpt = find(cut_part2.begin(), cut_part2.end(), random_chosen[chrome1].Get_Bit(j));
                while (cpt != cut_part2.end()) // 仍存在冲突
                {
                    random_chosen[chrome1].Set_Bit(j, cut_part1[cpt - cut_part2.begin()]);             // 映射赋值
                    cpt = find(cut_part2.begin(), cut_part2.end(), random_chosen[chrome1].Get_Bit(j)); // 继续针对该点寻找
                }
            }

            for (int j = 0; j < break_point; j++)
            {
                vector<int>::iterator cpt = find(cut_part1.begin(), cut_part1.end(), random_chosen[chrome2].Get_Bit(j));
                while (cpt != cut_part1.end()) // 仍存在冲突
                {
                    random_chosen[chrome2].Set_Bit(j, cut_part1[cpt - cut_part1.begin()]);             // 映射赋值
                    cpt = find(cut_part1.begin(), cut_part1.end(), random_chosen[chrome2].Get_Bit(j)); // 继续针对该点寻找
                }
            }
            ...
            if (c1x1 > x1max || c1x1 < x1min || c1x2 > x2max || c1x2 < x2min) // 个体不合法，恢复
            {
                random_chosen[chrome1].Set_Bvector(backup1);
                i--;
            }
            if (c2x1 > x1max || c2x1 < x1min || c2x2 > x2max || c2x2 < x2min)
            {
...
int main()
{
    ...
    for (int i = 0; i < num_of_iteration; i++)
    {
        ...
        // 按照适应度由大到小排序
        std::sort(origin.begin(), origin.end(), Fitness_Greater);
