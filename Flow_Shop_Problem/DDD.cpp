...
#define mutation_probability 0.1 // 变异概率 == 0.1
#define population_size 20       // 种群规模 == 20
#define num_of_iter 50           // 迭代次数 == 50
#define chosen_num 20            // 轮盘赌选择个体数
...
// 初始化种群
int initialize(vector<vector<Job>> &orders, vector<Job> &single, vector<Job> &job_finder)
{
...
int jobs_proc_time(vector<vector<Job>> &orders)
{
    for (int i = 0; i < population_size; i++)
    {
...
// 计算种群中每个个体的适应度
int chrome_fitnesses(vector<vector<Job>> &orders, vector<Info> &chrome_list1, int chrome_num)
{
    for (int i = 0; i < chrome_num; i++)
    {
        double fitness;
        int maxc = orders[i][jobs_num - 1].real_proc_t$[mach_num]; // 该个体对应的最大流程时间
        fitness = 1.0 / (maxc * 1.0);
...
// 轮盘赌选择 + 复制
int select(vector<vector<Job>> &orders, vector<Info> &chrome_list1, vector<vector<Job>> &random_chosen, vector<Info> &chrome_list2) // 个体适应度、种群
{
    // 计算选择概率分母
    ...
    while (chosen_cnt < chosen_num)
    {
        ...
        for (int j = 0; j < population_size; j++)
            if (chrome_list1[j].cumula_prob >= random_lf_1) // 选中
            {
                ...
                Info tt = chrome_list1[j];
                tt.id = chosen_cnt;         // 编号信息对应着 random_chosen 种群的个体的下标
                chrome_list2.push_back(tt); // 转存到新的种群信息表中
                ++chosen_cnt;
                break;
            }
    }
...
// 1 - 断点交叉算子
int cross_over(vector<vector<Job>> &random_chosen, vector<Info> &chrome_list2)
{
    ...
    // 两个一组进行交叉，真正最多执行 10次
    for (int i = 0; i < chosen_num / 2; i++) //chosen_num / 2
    {
        double cross_g = u1(e);
        printf("--%lf--\n", cross_g);
        ...
        if (cross_probability >= cross_g)
        {
            ...
            printf("Cross chromes group: %d %d.\n", chrome1, chrome2);
            printf("They are: \n");
            printf("$$$$$$$$$$\n");
            for (int j = 0; j < jobs_num; j++)
            {
                printf("%d ", random_chosen[chrome1][j].id$);
            }
            printf("\n");
            for (int j = 0; j < jobs_num; j++)
            {
                printf("%d ", random_chosen[chrome2][j].id$);
            }
            printf("\n");
            printf("$$$$$$$$$$\n");
            ...
            printf("Cut part1:\n");
            for (int j = 0; j < cross_len; j++)
            {
                printf("%d ", cut_part1[j].GetId());
            }
            printf("\n");
            printf("Cut part2:\n");
            for (int j = 0; j < cross_len; j++)
            {
                printf("%d ", cut_part2[j].GetId());
            }
            printf("\n");

            printf("\n----------Before solve: ------------\n");
            for (int k = 0; k < jobs_num; k++)
            {
                printf("%d ", random_chosen[chrome1][k].id$);
            }
            for (int k = 0; k < jobs_num; k++)
            {
                printf("%d ", random_chosen[chrome2][k].id$);
            }
...
int main()
{
    ...
    // 轮盘赌选择
    select(orders, chrome_list1, random_chosen, chrome_list2);
    ...
    /*     for (int i = 0; i < population_size; i++)
    {
        for (int j = 0; j < jobs_num; j++)
        {
            printf("%d ", random_chosen[i][j].GetId());
        }
        printf("\n");
    } */
    // 检查随机选择种群
    ...
    // 针对轮盘赌形成的新种群执行交叉操作
    cross_over(random_chosen, chrome_list2);
